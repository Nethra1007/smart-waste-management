...
Features: Login, Theme Customisation (Green Theme, Dark/Light Toggle), SQLite Historical Data, Live Dashboard
"""
...
THEMES = {
    "Light": {
        ...
        "text":        "#1B2428",
        "subtext":     "#4A5568",
        "header_bg":   "#1B5E20",      # dark green for sidebar header
        "header_text": "#FFFFFF",
...
# ─────────────────────────────────────────────
# CUSTOM CSS / THEME (No background image)
# ─────────────────────────────────────────────
def apply_theme(theme_name: str):
    """Inject CSS for the selected theme (no background image)."""
    t = THEMES[theme_name]
    st.markdown(f"""
    <style>
    .stApp {{
...
def show_login():
    ...
    st.markdown("""
        <div style='text-align:center; padding:60px 0 10px 0;'>
            ...
            <h1 style='color:#2E7D32; margin-bottom:4px;'>Smart Waste Analytics</h1>
    ...
    with col:
        st.markdown("""
            <div style='background:white;border-radius:16px;padding:32px 28px;
                ...
                <h3 style='color:#2E7D32;text-align:center;margin-bottom:20px;'>🔐 Login</h3>
            </div>
        """, unsafe_allow_html=True)
...
def render_dashboard():
    ...
    # Apply theme (no background image)
